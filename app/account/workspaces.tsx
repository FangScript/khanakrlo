...
function WorkspaceCardView({ card, onApply }: { card: WorkspaceCard; onApply: () => void }) {
  ...
  const description = card.workspaceType === "customer" ? "Order, track deliveries, and manage your account." : isBusiness ? "Create and run your Restaurant or Cloud Kitchen directly." : "Deliver orders after your rider application is approved.";
  const icon = card.workspaceType === "customer" ? "restaurant" : isBusiness ? "storefront" : "directions-bike";
  const isActive = card.status === "active";
  const canApply = isBusiness ? card.status !== "active" && card.status !== "suspended" : card.workspaceType !== "customer" && ["not_started", "draft", "changes_required"].includes(card.status);
  const openWorkspace = () => {
