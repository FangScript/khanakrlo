...
export default function BusinessCatalogueScreen() {
  ...
  if (catalogue.error || !data) {
    return <ScreenContainer edges={["top", "bottom", "left", "right"]}><View style={styles.accessBlock}><MaterialIcons name="storefront" size={32} color="#064B2C" /><Text style={styles.title}>Finish Business setup</Text><Text style={styles.accessCopy}>Restaurant menu management opens as soon as you complete setup. No Admin approval is needed to add dishes, set prices, or publish a customer menu.</Text><Pressable onPress={() => router.push("/business/onboarding" as never)} style={({ pressed }) => [styles.primaryAction, pressed && styles.pressed]}><Text style={styles.actionText}>Open Business setup</Text></Pressable><Pressable onPress={() => router.replace("/business/home" as never)} style={({ pressed }) => [styles.textAction, pressed && styles.pressed]}><Text style={styles.textActionLabel}>Back to Business</Text></Pressable></View></ScreenContainer>;
  }
