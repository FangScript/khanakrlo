  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileCard}><View style={styles.avatar}><Text style={styles.avatarText}>SA</Text></View><View style={styles.verified}><MaterialIcons name="verified" size={14} color="#17683A" /><Text style={styles.verifiedText}>Active rider</Text></View><Text style={styles.name}>Saad Ahmed</Text><Text style={styles.city}>Islamabad · Bike delivery partner</Text><Text style={styles.phone}>+92 312 876 5432</Text></View>
        <View style={styles.availabilityCard}><View><Text style={styles.availabilityTitle}>{available ? "You’re online" : "You’re offline"}</Text><Text style={styles.availabilitySubtitle}>{available ? "Ready to receive delivery requests." : "Go online when you’re ready to deliver."}</Text></View><Switch value={available} onValueChange={setAvailable} trackColor={{ false: "#D8DDD8", true: "#9EDAB0" }} thumbColor={available ? "#168A4A" : "#FFFFFF"} /></View>
