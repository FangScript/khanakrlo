...
const riderRows = [
  ...
  { icon: "account-balance-wallet" as const, title: "Cash account", subtitle: "Commission and COD custody are tracked here" },
  { icon: "health-and-safety" as const, title: "Safety & support", subtitle: "Emergency help and rider support" },
];

export default function RiderProfileScreen() {
  const { profile, hasHydrated, hydrateRiderSession } = useRiderStore();
  const availability = trpc.orders.riderAvailability.useQuery(undefined, { retry: false });
  const setAvailability = trpc.orders.setRiderAvailability.useMutation({ onSuccess: () => void availability.refetch() });
  const cashAccount = trpc.orders.riderCashAccount.useQuery(undefined, { retry: false });
  const custody = trpc.orders.riderCashCustodySummary.useQuery(undefined, { retry: false });
  const remit = trpc.orders.remitRiderCash.useMutation({ onSuccess: () => { void cashAccount.refetch(); void custody.refetch(); } });
  const isAvailable = availability.data?.status === "online";
  const accountBalance = cashAccount.data?.account.balanceMinor ?? 0;
  const openCustody = custody.data?.openCustodyMinor ?? 0;
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.availabilityCard}><View><Text style={styles.availabilityTitle}>{isAvailable ? "You’re online" : "You’re offline"}</Text><Text style={styles.availabilitySubtitle}>{isAvailable ? "You can receive new delivery offers." : "Go online when you’re ready to receive offers."}</Text></View><Switch value={isAvailable} disabled={availability.isLoading || setAvailability.isPending} onValueChange={(value) => void setAvailability.mutateAsync({ status: value ? "online" : "offline" })} trackColor={{ false: "#D8DDD8", true: "#9EDAB0" }} thumbColor={isAvailable ? "#168A4A" : "#FFFFFF"} /></View>
        <View style={styles.cashCard}><View style={styles.cashIcon}><MaterialIcons name="account-balance-wallet" size={20} color="#064B2C" /></View><View style={{ flex: 1 }}><Text style={styles.cashTitle}>Rider Cash Account</Text><Text style={styles.cashAmount}>PKR {(accountBalance / 100).toLocaleString("en-PK", { maximumFractionDigits: 2 })}</Text><Text style={styles.cashText}>Open COD custody: PKR {(openCustody / 100).toLocaleString("en-PK", { maximumFractionDigits: 2 })}. Commission is reserved when you accept a COD job.</Text>{openCustody > 0 ? <Pressable disabled={remit.isPending} onPress={() => void remit.mutateAsync({ amountMinor: openCustody })} style={({ pressed }) => [styles.remitButton, pressed && { opacity: 0.85 }, remit.isPending && { opacity: 0.55 }]}><Text style={styles.remitText}>Settle PKR {(openCustody / 100).toLocaleString("en-PK", { maximumFractionDigits: 2 })}</Text><MaterialIcons name="payments" size={15} color="#FFFFFF" /></Pressable> : null}</View></View>
