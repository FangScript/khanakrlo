...
export default function RiderDeliveryDetailScreen() {
  ...
  const { hasHydrated, hydrateRiderSession, getDelivery, updateDeliveryStatus, setDeliveryCashCollected } = useRiderStore();
  ...
  const advance = () => {
    ...
    if (nextStatus === "delivered" && delivery.cashToCollect > 0 && !delivery.cashCollected) {
      Alert.alert("Confirm cash collection", `Mark ${formatRupees(delivery.cashToCollect)} as collected before completing this COD delivery.`);
      return;
    }
    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <View style={styles.screen}><SuccessToast message={successMessage} />
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          {isPickedUp && delivery.cashToCollect > 0 ? <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: delivery.cashCollected }} onPress={() => setDeliveryCashCollected(delivery.id, !delivery.cashCollected)} style={[styles.cashCard, delivery.cashCollected && styles.cashCardChecked]}><View style={[styles.cashIcon, delivery.cashCollected && styles.cashIconChecked]}><MaterialIcons name={delivery.cashCollected ? "check" : "payments"} size={17} color={delivery.cashCollected ? "#FFFFFF" : "#9E5B00"} /></View><View style={{ flex: 1 }}><Text style={styles.proofTitle}>{delivery.cashCollected ? "Cash collection recorded" : `Collect ${formatRupees(delivery.cashToCollect)}`}</Text><Text style={styles.proofText}>{delivery.cashCollected ? "Keep the cash separate for operations reconciliation." : "Tap after receiving the exact COD amount from the customer."}</Text></View></Pressable> : null}
          {isPickedUp ? <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: proofConfirmed }} onPress={() => setProofConfirmed((value) => !value)} style={[styles.proofCard, proofConfirmed && styles.proofCardChecked]}><View style={[styles.proofCheck, proofConfirmed && styles.proofCheckChecked]}>{proofConfirmed ? <MaterialIcons name="check" size={17} color="#FFFFFF" /> : null}</View><View style={{ flex: 1 }}><Text style={styles.proofTitle}>Proof of delivery confirmed</Text><Text style={styles.proofText}>I verified the customer handoff and completed the handover safely.</Text></View></Pressable> : null}
        </ScrollView>
