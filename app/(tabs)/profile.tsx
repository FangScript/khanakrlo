...
const profileRows = [
  ...
  { icon: "notifications", title: "Notifications", subtitle: "Order updates and offers", message: "View your in-app order, support, and finance notifications.", route: "/notifications" },
  { icon: "help", title: "Help center", subtitle: "Order-linked support and feedback", message: "Open a customer support case or reply to an existing one.", route: "/support" },
] as const;
...
export default function ProfileScreen() {
  ...
  const signOut = () => Alert.alert("Log out?", "You will need to continue with Google again to access your account.", [{ text: "Cancel", style: "cancel" }, { text: "Log out", style: "destructive", onPress: () => { void logout().finally(() => { clearCustomerSession(); router.replace("/auth/login" as never); }); } }]);
  return <ScreenContainer><View style={styles.screen}><Text style={styles.title}>Profile</Text><View style={styles.profileCard}><View style={styles.avatar}><Text style={styles.avatarText}>{name.slice(0, 1).toUpperCase()}</Text></View><View style={styles.profileText}><Text style={styles.name}>{name}</Text><Text style={styles.phone}>{phone || "Google-authenticated account"}</Text></View><Pressable onPress={() => Alert.alert("Profile", "Contact details can be updated without changing your Google sign-in.")} style={({ pressed }) => [styles.editButton, pressed && styles.pressed]}><MaterialIcons name="edit" size={18} color="#064B2C" /></Pressable></View><Text style={styles.sectionLabel}>YOUR ACCOUNT</Text>{profileRows.map((row) => <Pressable key={row.title} onPress={() => router.push(row.route as never)} style={({ pressed }) => [styles.row, pressed && styles.pressed]}><View style={styles.rowIcon}><MaterialIcons name={row.icon} size={20} color="#064B2C" /></View><View style={styles.rowText}><Text style={styles.rowTitle}>{row.title}</Text><Text style={styles.rowSub}>{row.subtitle}</Text></View><MaterialIcons name="chevron-right" size={23} color="#879187" /></Pressable>)}<Pressable accessibilityRole="button" onPress={signOut} style={({ pressed }) => [styles.logoutButton, pressed && styles.pressed]}><MaterialIcons name="logout" size={19} color="#B52C25" /><Text style={styles.logoutText}>Log out</Text></Pressable><View style={styles.brandFooter}><KhanaKarLoMark size={34} /><View><Text style={styles.footerTitle}>Khaana <Text style={styles.footerOrange}>KarLo</Text></Text><Text style={styles.footerSub}>Pakistan ka apna food app</Text></View></View></View></ScreenContainer>;
}
