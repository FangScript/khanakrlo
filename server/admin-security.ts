...
export async function beginAdminMfaEnrollment(userId: number, req: Request) {
  ...
  await db.insert(adminMfaEnrollments).values({ userId, secretCiphertext: encryptSecret(secret), status: "pending", createdAt: now, confirmedAt: null, disabledAt: null }).onConflictDoUpdate({ target: adminMfaEnrollments.userId, set: { secretCiphertext: encryptSecret(secret), status: "pending", confirmedAt: null, disabledAt: null } });
...
export async function createAdminIpAllowlistRule(userId: number, input: { cidr: string; label: string }) { const { db, staffRole } = await ensureAdminUser(userId); if (staffRole !== "senior_operations") throw new AdminWebSecurityError("Only senior operators can manage network allowlisting."); const [created] = await db.insert(adminIpAllowlist).values({ cidr: input.cidr, label: input.label, status: "active", createdByUserId: userId }).returning({ id: adminIpAllowlist.id }); return { id: created.id }; }
