...
export async function createSupportTicket(userId: number, input: { orderId?: number; category: "order" | "delivery" | "payment" | "account" | "other"; subject: string; message: string }) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [createdTicket] = await tx.insert(supportTickets).values({ customerUserId: userId, ...input }).returning({ id: supportTickets.id });
...
export async function addCustomerSupportMessage(userId: number, input: { ticketId: number; body: string }) {
  ...
  const result = await db.transaction(async (tx) => {
    const [messageResult] = await tx.insert(supportTicketMessages).values({ ticketId: ticket.id, authorUserId: userId, authorType: "customer", visibility: "customer_visible", body: input.body }).returning({ id: supportTicketMessages.id });
...
export async function createRefundRequest(userId: number, input: { ticketId: number; requestedMinor: number; reason: string }) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [refundResult] = await tx.insert(refundRequests).values({ orderId: order.id, supportTicketId: ticket.id, customerUserId: userId, requestedMinor: input.requestedMinor, reason: input.reason, status: "requested" }).returning({ id: refundRequests.id });
    const refundRequestId = refundResult.id;
...
export async function updateNotificationPreferences(userId: number, input: { orderUpdatesEnabled: boolean; supportUpdatesEnabled: boolean; promotionsEnabled: boolean }) { const db = await dbOrThrow(); await db.insert(notificationPreferences).values({ userId, ...input }).onConflictDoUpdate({ target: notificationPreferences.userId, set: { ...input, updatedAt: new Date() } }); return getNotificationPreferences(userId); }
